...
根据JDK动态代理机制，调用被代理对象的方法之后实际上执行的就是代理对象的invoke方法。接下来，是构造一个MapperMethod对象,这个对象封装了Mapper接口中对应的方法信息以及对应的sql语句信息。这里面就会把要执行的sql语句，请求参数，方法返回值全部解析封装成MapperMethod对象，然后后面就可以开始准备执行sql语句了。

源代码如下。可以看到如果是select语句，还要根据返回值类型做不同的处理。
...
```mermaid
sequenceDiagram
    MapperProxy->>MapperProxy: cachedInvoker()
	MapperProxy->>MapperMethod: execute()<br>判断SQL类型
	MapperMethod->>SqlSession: 查询方法
	SqlSession->>Executor: 查询方法
	Executor->>SimpleExecutor: 查询方法
	SimpleExecutor->>BaseExecutor: 查询方法
	BaseExecutor->>StatementHandler: 查询方法
```
...
封装结果集。可以封装成多种类型可以是基本数据类型，也可以是Map、List、POJO类型复杂数据类型。封装结果集的过程就和JDBC封装结果集是一样的。也有两个常用的属性resultType和resultMap。
